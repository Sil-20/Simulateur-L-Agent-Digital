...
def generate_pdf_report(profil: ProfilProspect, sit: SituationActuelle,
    ...
    C_SKY       = colors.HexColor("#E8F6FB")   # bleu ciel très doux pour fond de section
    C_SKY_MED   = colors.HexColor("#CBE8F2")   # bleu ciel moyen pour titres
    ...
    page2_s  = ps("P2", fontSize=10, leading=14, fontName="Helvetica-Bold",
                  textColor=C_NAVY, spaceAfter=4)

    story = []

    # ── Dimensions utiles ─────────────────────────────────────────────────────
    page_w   = A4[0] - 2.8 * cm   # 17.5 cm
    COL_LBL  = 4.1 * cm
    COL_VAL  = 2.6 * cm
    COL_TBL  = COL_LBL + COL_VAL  # 6.7 cm
    COL_GAP  = 0.25 * cm
    COL_BAR  = page_w - 2 * COL_TBL - 2 * COL_GAP  # ~3.85 cm

    # ─────────────────────────────────────────────────────────────────────────
    # HEADER (commun aux 2 pages via une fonction)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def make_header(page_label):
        ...
        txt_col = Table([
            [Paragraph(f"Simulateur <font color='#2A93B5'>ROI Signature+</font>  "
                       f"<font color='#8AACBA' size='10'>{page_label}</font>", title_s)],
        ...
        row = Table([[logo_el, txt_col]],
                    colWidths=[logo_w + 0.4*cm, page_w - logo_w - 0.4*cm])
    ...
    def make_footer():
        return [
            ...
            Paragraph(
                f"l'agentdigital  \u00b7  Simulateur ROI Signature+ v4  \u00b7  {date_str}  \u00b7  "
                "Projections theoriques. Ne constituent pas une garantie de performance.",
                foot_s,
            ),
        ]

    # ── Bandeau section léger : fond bleu ciel doux, texte marine ──────────
    def section_banner(title, w_cm=17.5):
        ...
        d.add(Rect(0, 0, w, h,
                   fillColor=C_SKY_MED,
                   strokeColor=C_BLUE, strokeWidth=0.8))
        d.add(Rect(0, 0, 5, h, fillColor=C_BLUE, strokeColor=None))
        d.add(String(14, h / 2 - 5, title,
                     fontName="Helvetica-Bold", fontSize=9.5,
                     fillColor=colors.HexColor(NAVY)))
        return d

    # ─────────────────────────────────────────────────────────────────────────
    # PAGE 1 : KPI hero + Module 1 Relance
    # ─────────────────────────────────────────────────────────────────────────
    story.append(make_header("Page 1 / 2"))
    story.append(HRFlowable(width="100%", thickness=2.5, color=C_BLUE,
    ...
    annuel_rel_draw = _fe_draw(rel.ca_perdu_annuel)
    annuel_sc_draw  = _fe_draw(ca_rel_sc * 12)
    ...
    hero = _kpi_block_drawing([
        ...
        ("GAIN NET ANNUEL ESTIME",
         _fe_draw(comb['gain_net_mensuel'] * 12) + " EUR",
    ...
    rel_data = [
        ...
        ["CA perdu annuel",        _fe_plain(rel.ca_perdu_annuel) + " EUR"],
    ...
    # Layout page 1 : 2 graphiques à gauche (bar + pie), 2 tableaux à droite
    CHART_HALF_W = 5.6 * cm   # largeur de chaque graphique (bar et pie)
    GAP_CHARTS   = 0.25 * cm
    CHARTS_TOTAL = CHART_HALF_W * 2 + GAP_CHARTS  # ~11.45 cm
    TBL_W = (page_w - CHARTS_TOTAL - 2 * COL_GAP) / 2  # ~3.0 cm chacun

    t_sit = _make_table(sit_data, [TBL_W * 0.63, TBL_W * 0.37], highlight_rows=[3])
    t_rel = _make_table(rel_data, [TBL_W * 0.63, TBL_W * 0.37],
                        highlight_rows=[len(rel_data) - 2])
    ...
    bar_rel = _pdf_bar_chart(
        ...
        w_cm=CHART_HALF_W / cm, h_cm=CHART_H,
    )

    devis_relances = sit.devis_envoyes - rel.devis_sous_exploites
    ...
    row1 = Table(
        [[bar_rel, Spacer(GAP_CHARTS, 1), pie_rel, Spacer(COL_GAP, 1), t_sit, Spacer(COL_GAP, 1), t_rel]],
        colWidths=[CHART_HALF_W, GAP_CHARTS, CHART_HALF_W, COL_GAP, TBL_W, COL_GAP, TBL_W]
    )
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # PAGE 2 : Module 2 Acquisition + Vision combinée + Conclusion
    # ─────────────────────────────────────────────────────────────────────────
    ...
    comb_data = [
        ...
        ["CA acquisition / mois", _fe_plain(comb['ca_acquisition_mensuel']) + " EUR"],
    ...
    # Layout page 2 : funnel à gauche + 2 tableaux à droite
    FUNNEL_W = CHART_HALF_W * 2 + GAP_CHARTS   # même largeur que les 2 graphiques p1
    TBL2_W   = TBL_W

    t_acq  = _make_table(acq_data,  [TBL2_W * 0.63, TBL2_W * 0.37], highlight_rows=[7])
    t_comb = _make_table(comb_data, [TBL2_W * 0.63, TBL2_W * 0.37], highlight_rows=[4, 7])

    funnel = _pdf_funnel_chart(
        ["Clics", "Leads", "Qualifies", "Devis", "Ventes"],
        [acq.clics, acq.leads, acq.leads_qualifies, acq.devis, acq.ventes],
        w_cm=FUNNEL_W / cm, h_cm=11.0,
    )

    row2 = Table(
        [[funnel, Spacer(COL_GAP, 1), t_acq, Spacer(COL_GAP, 1), t_comb]],
        colWidths=[FUNNEL_W, COL_GAP, TBL2_W, COL_GAP, TBL2_W]
    )
    ...
    concl_body = (
        ...
        f"soit {_fe_plain(ca_rel_sc * 12)} EUR sur 12 mois.<br/><br/>"
        ...
        f"<b>{_fe_plain(acq.ca_additionnel)} EUR de CA additionnel / mois</b>.<br/><br/>"
        ...
        f"Retour sur investissement du setup : <b>{payback_str}</b>.  "
        f"ROI global estime : <b>{comb['roi_global']:.1f}x</b>."
    )

    concl_para = Paragraph(concl_body, concl_s)
    concl_tbl = Table([[concl_para]], colWidths=[page_w])
