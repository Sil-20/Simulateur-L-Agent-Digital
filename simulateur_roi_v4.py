...
def generate_pdf_report(profil: ProfilProspect, sit: SituationActuelle,
    ...
    def make_kpi_hero(kpis, h=3.4 * cm):
        """
        Bloc KPI hero 4 colonnes - fond BG_LIGHT + bordure bleue (identique v3).
        kpis = [(label, value_str, sub_str), ...]
        """
        w = PW
        d = Drawing(w, h)
        # Fond + bordure (style v3 exact)
        d.add(Rect(0, 0, w, h,
                   fillColor=C_BG,
                   strokeColor=C_BLUE,
        ...
        for i, (lbl, val, sub) in enumerate(kpis):
            ...
            if i > 0:
                d.add(Line(x0, h * 0.08, x0, h * 0.92,
                           strokeColor=C_BDR, strokeWidth=0.7))
            # Label petit gris majuscules
            d.add(String(x0 + pad, h - 16, lbl,
                         fontName="Helvetica-Bold", fontSize=6.5,
                         fillColor=colors.HexColor(GRAY_TEXT)))
            # Valeur grande bleue - taille adaptee
            vlen  = len(val)
    ...
    def make_funnel(labels, values, w, h):
        """Entonnoir horizontal style v3 adapte."""
        d = Drawing(w, h)
        mx     = values[0] if values[0] > 0 else 1
        n      = len(labels)
        top_r  = 10
        ...
        bar_h  = slot * 0.66
        max_bw = w * 0.55
        fcols  = [BLUE_DARK, BLUE_AD, BLUE_AD, BLUE_AD, GREEN]
        for i, (lbl, val) in enumerate(zip(labels, values)):
            col   = fcols[i] if i < len(fcols) else BLUE_AD
    ...
    story.append(make_kpi_hero([
        ("CA PERDU ANNUEL",
         _fe_plain(rel.ca_perdu_annuel) + " EUR",
    ...
    pie_rel = make_pie_chart([
        (rel.devis_sous_exploites, ORANGE,  "Non relances", f"{rel.devis_sous_exploites:.0f} devis"),
        (devis_relances,           BLUE_AD, "Relances",     f"{devis_relances:.0f} devis"),
    ], CHART_W, CHART_H)
    ...
    pie_acq = make_pie_chart([
        (cout_annuel,  GRAY_MID, "Cout annuel",   _fe_plain(cout_annuel) + " EUR"),
        (max(0, gain_annuel), BLUE_AD, "Gain net ann.", _fe_plain(max(0, gain_annuel)) + " EUR"),
    ], PIE_W, CHART_H2)
