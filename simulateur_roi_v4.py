...
def generate_pdf_report(profil: ProfilProspect, sit: SituationActuelle,
    ...
    """
    Rapport PDF 2 pages - design professionnel oriente ROI.

    Page 1 : Header · KPI Hero (3 colonnes) · Module Relance
             [bar CA recuperable] + [pie ROI Signature+] · Tableaux
    Page 2 : Header · Module Acquisition
             [barres horizontales conversion] + [pie ROI Acquisition] · Tableaux · Conclusion

    Modifications v4 -> v5 :
    - Page 1 : pie "repartition devis" remplace par pie "ROI Signature+"
    - Page 2 : entonnoir vertical remplace par barres horizontales de conversion
    - Page 2 : pie "cout vs gain" remplace par pie "ROI Acquisition"
    - Espacement global augmente (sections plus aerees)
    - Mise en page plus equilibree sur les 2 pages
    """
    ...
    buffer = io.BytesIO()
    LM = RM = 1.6 * cm
    TM = 1.6 * cm           # marge top legerement augmentee
    BM = 1.8 * cm           # marge bas augmentee → aspect rapport conseil
    ...
    C_BLUE   = colors.HexColor("#2A93B5")
    C_BLUE2  = colors.HexColor("#1C6E8A")
    C_BLUE3  = colors.HexColor("#5BB3CC")
    C_BLUE4  = colors.HexColor("#A8D8E8")
    C_BLUE5  = colors.HexColor("#D6EEF5")
    ...
    COL_BAR2 = "#2A93B5"
    COL_BAR3 = "#1C6E8A"
    COL_PIE1 = "#A8D8E8"   # part "cout" - bleu clair (moins important)
    COL_PIE2 = "#2A93B5"   # part "gain" - bleu principal (impact visuel fort)
    # Nuances pour barres horizontales de conversion
    COL_CONV = ["#1C6E8A", "#2A93B5", "#2A93B5", "#5BB3CC", "#A8D8E8"]
    ...
    sec_s    = ps("SE", fontSize=6.5, leading=8, fontName="Helvetica-Bold",
                  ...
                  spaceBefore=0, spaceAfter=6, letterSpacing=1.2)
    ...
    # Calculs ROI pour les deux pie charts
    cout_sig_annuel  = 249 * 12                        # forfait relance annuel
    gain_sig_annuel  = ca_rel_sc * 12                  # gain relance annuel scenario
    cout_acq_annuel  = comb['cout_total_mensuel'] * 12 # cout total acquisition annuel
    gain_acq_annuel  = max(0, acq.ca_additionnel * 12) # gain brut acquisition annuel
    ...
    def make_kpi_hero(kpis, h=3.6 * cm):
        """
        KPI Hero N colonnes.
        Fond bleu tres clair, bordure bleue, accent strip en haut, valeurs en bleu.
        """
        w = PW
        d = Drawing(w, h)
        d.add(Rect(0, 0, w, h,
    ...
    def make_pie_roi(cout, gain, label_cout, label_gain, titre_roi, w, h):
        """
        Pie chart ROI : part "cout" (bleu clair) vs part "gain" (bleu principal).
        Affiche le ratio ROI au centre du pie.
        Labels commerciaux clairs dans la legende a droite.
        """
        d    = Drawing(w, h)
        total = cout + gain
        if total <= 0:
        ...
        slices = [
            (cout, COL_PIE1, label_cout, _fe_plain(cout) + " EUR"),
            (gain, COL_PIE2, label_gain, _fe_plain(gain) + " EUR"),
        ]

        start = 90.0
        for val, col, lbl, val_str in slices:
            angle = (val / total) * 360.0
            end   = start - angle
            d.add(Wedge(cx, cy, r, end, start,
                        fillColor=colors.HexColor(col),
                        strokeColor=C_WHITE, strokeWidth=1.5))
            # Label % dans chaque part
        ...
        # Ratio ROI au centre (trou "donut style" visuel)
        roi_val = gain / cout if cout > 0 else 0
        roi_txt = f"x{roi_val:.1f}"
        d.add(String(cx, cy - 4, roi_txt,
                     fontName="Helvetica-Bold", fontSize=11,
                     fillColor=colors.HexColor(NAVY), textAnchor="middle"))
        d.add(String(cx, cy - 14, "ROI",
                     fontName="Helvetica", fontSize=6,
                     fillColor=colors.HexColor(GRAY_TEXT), textAnchor="middle"))
        ...
        leg_x = cx + r + 14
        leg_y = cy + 18
        for i, (val, col, lbl, val_str) in enumerate(slices):
            ly = leg_y - i * 30
            ...
            d.add(String(leg_x + 13, ly + 1, lbl,
                         fontName="Helvetica", fontSize=6.5,
                         fillColor=colors.HexColor(GRAY_TEXT)))
            d.add(String(leg_x + 13, ly - 10, val_str,
                         fontName="Helvetica-Bold", fontSize=7.5,
                         fillColor=colors.HexColor(NAVY)))

        # Titre du graphique en dessous
        d.add(String(w / 2, 2, titre_roi,
                     fontName="Helvetica-Bold", fontSize=6,
                     fillColor=colors.HexColor(BLUE_AD), textAnchor="middle"))
        return d

    def make_conversion_bars(labels, values, w, h):
        """
        NOUVEAU (remplace l'entonnoir vertical) :
        Barres HORIZONTALES de conversion - une par etape de l'entonnoir.
        Chaque barre montre la valeur absolue et le taux de conversion vs etape precedente.
        Structure : [label] [barre coloree proportionnelle] [valeur + %]
        Plus lisible en RDV qu'un entonnoir vertical.
        """
        d    = Drawing(w, h)
        mx   = values[0] if values[0] > 0 else 1
        n    = len(labels)

        label_w = 1.6 * cm          # largeur zone labels a gauche
        val_w   = 1.4 * cm          # largeur zone valeurs a droite
        bar_area= w - label_w - val_w - 8  # largeur utile pour les barres
        slot_h  = h / n
        bar_h   = slot_h * 0.60
        pad_top = (slot_h - bar_h) / 2

        for i, (lbl, val) in enumerate(zip(labels, values)):
            col   = COL_CONV[i] if i < len(COL_CONV) else COL_BAR2
            # Position Y : on commence en haut
            by    = h - (i + 1) * slot_h + pad_top
            bw    = max(bar_area * 0.04, (val / mx) * bar_area)

            # Label etape (gauche)
            d.add(String(label_w - 4, by + bar_h * 0.28, lbl,
                         fontName="Helvetica-Bold", fontSize=6.5,
                         fillColor=colors.HexColor(NAVY), textAnchor="end"))

            # Barre
            d.add(Rect(label_w, by, bw, bar_h,
                       fillColor=colors.HexColor(col), strokeColor=None, rx=2, ry=2))

            # Valeur absolue + % de conversion vs etape precedente
            if i == 0:
                pct_txt = "100%"
            else:
                prev = values[i - 1]
                pct_val = (val / prev * 100) if prev > 0 else 0
                pct_txt = f"{pct_val:.0f}%"

            vd = f"{int(round(val))}" if val >= 10 else f"{val:.1f}"
            d.add(String(label_w + bw + 5, by + bar_h * 0.28,
                         f"{vd}  ({pct_txt})",
                         fontName="Helvetica", fontSize=6.5,
                         fillColor=colors.HexColor(GRAY_TEXT)))

            # Ligne de separation legere entre etapes
            if i < n - 1:
                sep_y = h - (i + 1) * slot_h
                d.add(Line(label_w, sep_y, label_w + bar_area * 0.85, sep_y,
                           strokeColor=C_BDR, strokeWidth=0.4))
        return d
    ...
    # ── MODULE 1 – Graphiques Relance ─────────────────────────────────────────
    story.append(Paragraph(
        "MODULE 1  \u2013  RELANCE DEVIS  |  CA recuperable & ROI Signature+",
        sec_s))
    story.append(Spacer(1, 28))  # espace ajoute apres le titre du module (≈ 2 lignes)

    CHART_W = PW / 2 - 0.3 * cm
    CHART_H = 5.4 * cm

    # Graphique gauche : barres CA recuperable par scenario (inchange)
    bar_rel = make_bar_chart(
        ...
        [COL_BAR1, COL_BAR2, COL_BAR3],
        CHART_W, CHART_H,
    )

    # Graphique droit : NOUVEAU - pie ROI Signature+ (cout vs gain relance)
    pie_sig = make_pie_roi(
        cout=cout_sig_annuel,
        gain=gain_sig_annuel,
        label_cout="Forfait annuel",
        label_gain="Gain relance",
        titre_roi=f"ROI Signature+ (scenario {scenario})",
        w=CHART_W,
        h=CHART_H,
    )

    charts_row1 = Table(
        [[chart_col_wrap("CA RECUPERABLE SELON SCENARIO (EUR)", bar_rel, CHART_W + 0.3*cm),
          chart_col_wrap("ROI SIGNATURE+ : FORFAIT VS GAIN RELANCE", pie_sig, CHART_W + 0.3*cm)]],
        colWidths=[CHART_W + 0.3*cm, CHART_W + 0.3*cm])
    ...
    story.append(Spacer(1, 52))  # espace augmente entre les graphiques et le tableau (≈ 2 lignes sup.)
    ...
    # ── MODULE 2 – Graphiques Acquisition ────────────────────────────────────
    story.append(Paragraph(
        "MODULE 2  \u2013  ACQUISITION CLIENT  |  Conversion & ROI Acquisition",
        sec_s))

    CONV_W  = PW * 0.50              # barres conversion : 50% largeur
    PIE_W   = PW - CONV_W - 0.4 * cm
    CHART_H2 = 5.6 * cm

    # Graphique gauche : NOUVEAU - barres horizontales de conversion
    conv_bars = make_conversion_bars(
        ["Clics", "Leads", "Qualifies", "Devis", "Ventes"],
        [acq.clics, acq.leads, acq.leads_qualifies, acq.devis, acq.ventes],
        CONV_W, CHART_H2,
    )

    # Graphique droit : NOUVEAU - pie ROI Acquisition (cout vs gain acquisition)
    pie_acq_roi = make_pie_roi(
        cout=cout_acq_annuel,
        gain=gain_acq_annuel,
        label_cout="Cout acquisition",
        label_gain="Gain acquisition",
        titre_roi="ROI Acquisition (annuel)",
        w=PIE_W,
        h=CHART_H2,
    )

    charts_row2 = Table(
        [[chart_col_wrap("ENTONNOIR DE CONVERSION ACQUISITION", conv_bars, CONV_W + 0.2*cm),
          chart_col_wrap("ROI ACQUISITION : COUT VS GAIN ANNUEL", pie_acq_roi, PIE_W + 0.2*cm)]],
        colWidths=[CONV_W + 0.2*cm, PIE_W + 0.2*cm])
